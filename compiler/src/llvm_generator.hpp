...
public:
    ...
    GenScope( llvm::IRBuilder<> *builder, llvm::DIScope* debugScope=nullptr )
            : entryBlock( builder->GetInsertBlock() ), currentBlock(), debugScopeStack(),
              builder( builder ) {
        if ( debugScope )
            debugScopeStack.push( debugScope );
    }

    llvm::DIScope* debug_scope() const { return debugScopeStack.top(); }
    void push_debug_scope( llvm::DIScope* diScope ) { debugScopeStack.push( diScope ); }
    void pop_debug_scope() { debugScopeStack.pop(); }
...
class LlvmGenerationContext {
    ...
    std::map<const TxActualType*, llvm::DIType*> llvmDebugTypeMapping;
    ...
    /** represents this compilation unit (Tuplex compilation package) */
    llvm::DICompileUnit* _debugUnit;
    /** debug info builder */
    llvm::DIBuilder* _debugBuilder;
    /** represents the LllvmGenerationContext source file, for code generated by it */
    llvm::DIFile* _genDIFile;
...
public:
    ...
    inline llvm::DIBuilder* debug_builder() const {
        return this->_debugBuilder;
    }
    ...
    inline llvm::Type* get_llvm_type( const TxQualType qtype ) {
        return this->get_llvm_type( qtype.type() );
    }
    llvm::Type* get_llvm_type( const TxActualType* txType );

    inline llvm::DIType* get_debug_type( const TxQualType qtype ) {
        return this->get_debug_type( qtype.type() );
    }
    llvm::DIType* get_debug_type( const TxActualType* txType );
    ...
    /** To be run before all other codegen. */
    void init_codegen();

    /** To be run after all other codegen and before code is verified/dumped. */
...
/*--- adapters so that LLVM Type and Value can be printed to character streams ---*/
...
template<typename charT, typename traits>
std::basic_ostream<charT, traits> &
operator<<( std::basic_ostream<charT, traits> &lhs, const llvm::DIType& type ) {
    std::string tmp_str;
    llvm::raw_string_ostream rso( tmp_str );
    type.print( rso );
    return lhs << rso.str();
